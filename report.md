...
Further analysis using Shapley Additive Explanations (SHAP) provided similar results, also showing the importance for each severity class. It's worth noticing that, for the prediction of Severity 4, the presence of the word 'closed'and the distance are the most informative features, while the contribute of the other features seems to be marginal.
